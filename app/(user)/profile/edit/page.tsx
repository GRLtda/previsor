...
export default function EditProfilePage() {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            await refreshUser()
            // Redirect to public profile using nickname if acquired, else ID
            const newIdentifier = nickname || user.id
            router.push(`/@${newIdentifier}`)
        } catch (err) {
    ...
    const backLink = user.nickname ? `/@${user.nickname}` : `/profile/${user.id}`
                ...
                <main className="relative flex w-full flex-col">
                    ...
                    <div className="flex w-full flex-col gap-6">
                        ...
                        <div className="flex w-full flex-col gap-2">
                            ...
                            <div className="relative flex h-[46px] items-center rounded-lg border border-border bg-transparent">
                                <span className="flex h-full items-center justify-center pl-4 pr-1 text-muted-foreground font-medium">@</span>
                                <input
                                    id="nickname"
                                    ...
                                    className="h-full w-full bg-transparent pr-4 text-sm font-medium text-black placeholder:text-muted-foreground focus:outline-none dark:text-white"
                                    type="text"
                                />
                            </div>
                            <span className="text-xs text-muted-foreground">O nickname só pode conter letras minúsculas, números e underline (_).</span>
                        </div>
                        ...
                        <div className="flex w-full flex-col gap-2">
                            ...
                            <div className="flex w-full flex-col gap-3 lg:flex-row">
                                {/* Twitter */}
                                <div className="relative flex h-[46px] w-full items-center rounded-lg border border-border bg-transparent lg:flex-1">
