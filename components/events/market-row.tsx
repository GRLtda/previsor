...
export function MarketRow({ market: initialMarket, onYesClick, onNoClick }: MarketRowProps) {
    ...
    return (
        <div className="flex w-full items-center justify-between border-b border-border/40 py-3 sm:py-4 transition-colors hover:bg-white/[0.02] last:border-b-0 px-2 sm:px-4">
            ...
            {/* Middle - Percentage */}
            <div className="flex w-fit items-center justify-center px-2 sm:px-6 shrink-0">
                <span className="text-lg sm:text-2xl font-bold dark:text-white leading-none tracking-tight">
                    {yesPercent}%
                </span>
            </div>

            {/* Right side - Buttons */}
            <div className="flex w-fit items-center justify-end gap-1.5 sm:gap-3 shrink-0">
                <button
                    ...
                    className={cn(
                        "flex h-[36px] sm:h-[42px] flex-1 items-center justify-center gap-1 sm:gap-2 rounded-xl border font-bold transition-all duration-300 px-2 sm:px-3 text-[11px] sm:text-sm",
                        isMarketOpen
